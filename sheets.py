...
def update_deal_id(user_id, deal_id):
    """Обновляет deal_id для существующей строки клиента."""
    try:
        sheet = get_chats_sheet()
        rows = sheet.get_all_records()
        for idx, r in enumerate(rows, start=2):
            if int(r["user_id"]) == user_id:
                sheet.update_cell(idx, 6, str(deal_id))
                return
    except Exception as e:
...
def update_last_client_message(user_id):
    """Обновляет время последнего сообщения КЛИЕНТА и сбрасывает touch_number на 0."""
    try:
        sheet = get_chats_sheet()
        idx = _find_row_index(sheet, user_id)
        if idx:
            now_iso = datetime.now(timezone.utc).isoformat()
            sheet.update_cell(idx, 7, now_iso)
            sheet.update_cell(idx, 9, "0")
            sheet.update_cell(idx, 10, "active")
    except Exception as e:
        logging.error(f"Ошибка обновления last_client_message_time: {e}")


def update_last_manager_message(user_id):
    """Обновляет время последнего сообщения МЕНЕДЖЕРА."""
    try:
        sheet = get_chats_sheet()
        idx = _find_row_index(sheet, user_id)
        if idx:
            now_iso = datetime.now(timezone.utc).isoformat()
            sheet.update_cell(idx, 8, now_iso)
    except Exception as e:
        logging.error(f"Ошибка обновления last_manager_message_time: {e}")


def update_touch_number(user_id, touch_number):
    """Обновляет touch_number для клиента после успешной отправки касания."""
    try:
        sheet = get_chats_sheet()
        idx = _find_row_index(sheet, user_id)
        if idx:
